...
{
    void *voidPtr =
        GameOverlay::find_hook_trampoline (&VR_GetGenericInterface) (pchInterfaceVersion, peError);
    ...
    bool SteamVR_D3D::Init (const vr::ETextureType eType)
    {
        ...
        switch (eType)
        {
            case vr::TextureType_DirectX:
            {
                ...
                HRESULT hr =
                    d3d11Device_->CreateTexture2D (&displayDesc, nullptr, &d3d11OverlayTexture_);
                ...
                hr = d3d11Device_->CreateRenderTargetView (
                    d3d11OverlayTexture_.Get (), &rtvDesc, &d3d11RenderTargets_[0]);
                ...
                d3d11Renderer_.reset (new GameOverlay::d3d11_renderer (
                    d3d11Device_.Get (), d3d11RenderTargets_, screenWidth_, screenHeight_));
            ...
            case vr::TextureType_DirectX12:
            {
                ...
                HRESULT hr = d3d12Device_->CreateCommittedResource (&textureHeapType,
                    D3D12_HEAP_FLAG_NONE, &textureDesc, D3D12_RESOURCE_STATE_RENDER_TARGET, nullptr,
                ...
                hr = d3d12Device_->CreateDescriptorHeap (
                    &rtvHeapDesc, IID_PPV_ARGS (&d3d12RenderTargetHeap_));
                if (FAILED (hr))
                {
                    return false;
                }
                ...
                CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle (
                    d3d12RenderTargetHeap_->GetCPUDescriptorHandleForHeapStart ());

                d3d12Device_->CreateRenderTargetView (
                    d3d12RenderTargets_[0].Get (), nullptr, rtvHandle);
                rtvHandle.Offset (1, d3d12RtvHeapDescriptorSize_);

                d3d12Renderer_.reset (new GameOverlay::d3d12_renderer (d3d12Commandqueue_.Get (),
                    d3d12RenderTargetHeap_, d3d12RenderTargets_, d3d12RtvHeapDescriptorSize_, 1,
        ...
        switch (pTexture->eType)
        {
            ...
            case vr::TextureType_DirectX12:
            {
                ...
                vr::D3D12TextureData_t *d3d12TextureData =
                    static_cast<vr::D3D12TextureData_t *> (handle);
