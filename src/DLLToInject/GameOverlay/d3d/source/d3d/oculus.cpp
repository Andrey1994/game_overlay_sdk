...
{
    ovrResult result =
        GameOverlay::find_hook_trampoline (&ovr_CommitTextureSwapChain) (session, chain);
...
namespace CompositorOverlay
{
        ...
        switch (d3dVersion_)
        {
            ...
            case D3DVersion_12:
            {
                ...
                d3d12Renderer_ = std::make_unique<GameOverlay::d3d12_renderer> (
                    d3d12Commandqueue_.Get (), d3d12RenderTargetHeap_, d3d12RenderTargets_,
                    d3d12RtvHeapDescriptorSize_, bufferCount_, screenWidth_, screenHeight_);
    ...
    bool Oculus_D3D::Render (ovrSession session)
    {
        ...
        GameOverlay::find_hook_trampoline (&ovr_GetTextureSwapChainCurrentIndex) (
            session, swapchain_, &currentIndex);
    ...
    bool Oculus_D3D::CreateD3D11RenderTargets (ovrSession session)
    {
        ...
        GameOverlay::find_hook_trampoline (&ovr_GetTextureSwapChainLength) (
            session, swapchain_, &count);
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            rtvd.ViewDimension = textureDesc.SampleDesc.Count > 1 ?
                D3D11_RTV_DIMENSION_TEXTURE2DMS :
                D3D11_RTV_DIMENSION_TEXTURE2D;
            rtvd.Texture2D.MipSlice = 0;

            hr = d3d11Device_->CreateRenderTargetView (
                texture.Get (), &rtvd, &d3d11RenderTargets_[i]);
    ...
    bool Oculus_D3D::CreateD3D12RenderTargets (ovrSession session)
    {
        ...
        {
            hr = GameOverlay::find_hook_trampoline (&ovr_GetTextureSwapChainBufferDX) (session,
                swapchain_, buffer_index, IID_PPV_ARGS (&d3d12RenderTargets_[buffer_index]));
            ...
            d3d12Device_->CreateRenderTargetView (
                d3d12RenderTargets_[buffer_index].Get (), &rtvd, rtvHandle);
