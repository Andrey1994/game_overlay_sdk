...
extern "C"
{
    ...
    typedef enum ovrProjectionModifier_
    {
        ...
        /// After the projection transform is applied, far values stored in the depth buffer will be
        /// less
        ...
        /// When this flag is used, the zfar value pushed into ovrMatrix4f_Projection() will be
        /// ignored
        /// NOTE: Enable only if ovrProjection_FarLessThanNear is also enabled where the far
        /// clipping
        /// plane will be pushed to infinity.
        ovrProjection_FarClipAtInfinity = 0x04,

        /// Enable if the application is rendering with OpenGL and expects a projection matrix with
        /// a
        /// clipping range of (-w to w).
        /// Ignore this flag if your application already handles the conversion from D3D range (0 to
        /// w) to
        /// OpenGL.
    ...
    ///
    /// \param[in] timeoutMilliseconds Specifies a timeout to wait for HMD to be attached or 0 to
    /// poll.
    ///
...
#define OVR_HMD_CONNECTED_EVENT_NAME L"OculusHMDConnected"
    ...
    ovrMatrix4f_OrthoSubProjection (
        ovrMatrix4f projection, ovrVector2f orthoScale, float orthoDistance, float HmdToEyeOffsetX);
...
#undef ovr_CalcEyePoses
    ...
    ovr_CalcEyePoses (
        ovrPosef headPose, const ovrVector3f hmdToEyeOffset[2], ovrPosef outEyePoses[2]);
...
#undef ovr_GetEyePoses
    OVR_PUBLIC_FUNCTION (void)
    ovr_GetEyePoses (ovrSession session, long long frameIndex, ovrBool latencyMarker,
        const ovrVector3f hmdToEyeOffset[2], ovrPosef outEyePoses[2], double *outSensorSampleTime);
    OVR_PRIVATE_FUNCTION (void)
    ovr_GetEyePoses2 (ovrSession session, long long frameIndex, ovrBool latencyMarker,
        const ovrPosef HmdToEyePose[2], ovrPosef outEyePoses[2], double *outSensorSampleTime);
#define ovr_GetEyePoses ovr_GetEyePoses2

    /// Tracking poses provided by the SDK come in a right-handed coordinate system. If an
    /// application is passing in ovrProjection_LeftHanded into ovrMatrix4f_Projection, then it
    /// should also use this function to flip the HMD tracking poses to be left-handed.
    ///
    /// While this utility function is intended to convert a left-handed ovrPosef into a
    /// right-handed coordinate system, it will also work for converting right-handed to left-handed
    /// since the flip operation is the same for both cases.
    ///
    ...
    ovr_ReadWavFromBuffer (ovrAudioChannelData *outAudioChannel, const void *inputData,
        int dataSizeInBytes, int stereoChannelToUse);
    ...
    ovr_GenHapticsFromAudioData (ovrHapticsClip *outHapticsClip,
        const ovrAudioChannelData *audioChannel, ovrHapticsGenMode genMode);
