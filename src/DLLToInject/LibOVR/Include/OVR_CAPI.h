...
typedef enum ovrStatusBits_
{
    ovrStatus_OrientationTracked =
        0x0001,                         ///< Orientation is currently tracked (connected & in use).
...
typedef enum ovrTextureFormat_
{
    ...
    OVR_FORMAT_B5G5R5A1_UNORM =
        2, ///< Not currently supported on PC. Requires a DirectX 11.1 device.
    OVR_FORMAT_B4G4R4A4_UNORM =
        3, ///< Not currently supported on PC. Requires a DirectX 11.1 device.
...
typedef enum ovrButton_
{
    ...
    /// Right shoulder button on XBox controllers. Not present on Touch controllers or Oculus
    /// Remote.
...
typedef struct ovrInputState_
{
    ...
    /// Left and right hand trigger values (ovrHand_Left and ovrHand_Right), in the range 0.0
    /// to 1.0f. This has been formally named "Grip Button". We retain the name HandTrigger for
    /// backwards code compatibility. User-facing documentation should refer to it as the Grip
    /// Button or simply Grip.
    float HandTrigger[ovrHand_Count];

    /// Horizontal and vertical thumbstick axis values (ovrHand_Left and ovrHand_Right), in the
    /// range of -1.0f to 1.0f. Returns a deadzone (value 0) per each axis if the value on that axis
    ...
    /// Left and right hand trigger values (ovrHand_Left and ovrHand_Right), in the range 0.0
    /// to 1.0f. Does not apply a deadzone. Only touch applies a filter. This has been formally
    /// named "Grip Button". We retain the name HandTrigger for backwards code compatibility.
    /// User-facing documentation should refer to it as the Grip Button or simply Grip.
    float HandTriggerNoDeadzone[ovrHand_Count];

    /// Horizontal and vertical thumbstick axis values (ovrHand_Left and ovrHand_Right), in the
    /// range -1.0f to 1.0f Does not apply a deadzone or filter.
    ovrVector2f ThumbstickNoDeadzone[ovrHand_Count];

    /// Left and right finger trigger values (ovrHand_Left and ovrHand_Right), in range 0.0 to 1.0f.
    /// No deadzone or filter
    /// This has been formally named "Grip Button". We retain the name HandTrigger for backwards
    /// code compatibility. User-facing documentation should refer to it as the Grip Button or
    /// simply Grip.
    float IndexTriggerRaw[ovrHand_Count];

    /// Left and right hand trigger values (ovrHand_Left and ovrHand_Right), in the range 0.0
    /// to 1.0f. No deadzone or filter This has been formally named "Grip Button". We retain the
    /// name HandTrigger for backwards code compatibility. User-facing documentation should refer to
    /// it as the Grip Button or simply Grip.
    float HandTriggerRaw[ovrHand_Count];

    /// Horizontal and vertical thumbstick axis values (ovrHand_Left and ovrHand_Right), in the
    /// range -1.0f to 1.0f No deadzone or filter
...
{
    /// Time in seconds from last change to the parameters.
    /// For instance, if the pose changes, or a camera exposure happens, this struct will be
    /// updated.
...
extern "C"
{
    ...
    /// be called unless ovr_Initialize succeeds. A successful call to ovr_Initialize must be
    /// eventually followed by a call to ovr_Shutdown. ovr_Initialize calls are idempotent. Calling
    /// ovr_Initialize twice does not require two matching calls to ovr_Shutdown. If already
    /// initialized, the return value is ovr_Success.
    ///
    ...
    ///
    /// \param params Specifies custom initialization options. May be NULL to indicate default
    /// options
    ...
    ///         }; ovrResult result = ovr_Initialize(&initParams); if(OVR_FAILURE(result)) {
    ...
    /// @name HMD Management
    ///
    /// Handles the enumeration, creation, destruction, and properties of an HMD (head-mounted
    /// display).
    ///@{
    ...
    ///
    /// ovr_Initialize must have first been called in order for this to succeed, otherwise the
    /// returned trackerDescArray will be zero-initialized. The data returned by this function can
    /// change at runtime.
    ///
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    ///
    ...
    /// longer needed.
    /// A second call to ovr_Create will result in an error return value if the previous session has
    /// not been destroyed.
    ///
    /// \param[out] pSession Provides a pointer to an ovrSession which will be written to upon
    /// success. \param[out] pLuid Provides a system specific graphics adapter identifier that
    /// locates which graphics adapter has the HMD attached. This must match the adapter used by the
    /// application or no rendering output will be possible. This is important for stability on
    /// multi-adapter systems. An application that simply chooses the default adapter will not run
    /// reliably on multi-adapter systems. \return Returns an ovrResult indicating success or
    ...
    typedef struct ovrSessionStatus_
    {
        ...
        /// controller state). If this is false then the application is in the background (but
        /// possibly still visible) should hide any input representations such as hands.
    ...
    ovr_IsExtensionSupported (
        ovrSession session, ovrExtensions extension, ovrBool *outExtensionSupported);
    ...
    /// be redefined. All future tracking will report values relative to this new reference
    /// position. If you are using ovrTrackerPoses then you will need to call ovr_GetTrackerPose
    ...
    ///
    /// \return Returns an ovrResult indicating success or failure. In the case of failure, use
    ///         ovr_GetLastErrorInfo to get more information. Return values include but aren't
    ///         limited to:
    ///     - ovrSuccess: Completed successfully.
    ...
    ///         such as facing vertically. This can happen if the caller is directly feeding the
    ///         pose of a position-tracked device such as an HMD or controller into this function.
    ///
    /// \see ovrTrackingOrigin, ovr_GetTrackerPose, ovr_RecenterTrackingOrigin
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_SpecifyTrackingOrigin (ovrSession session, ovrPosef originPose);

    /// Clears the ShouldRecenter status bit in ovrSessionStatus.
    ///
    /// Clears the ShouldRecenter status bit in ovrSessionStatus, allowing further recenter requests
    /// to be detected. Since this is automatically done by ovr_RecenterTrackingOrigin and
    ...
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_GetDevicePoses (ovrSession session, ovrTrackedDeviceType *deviceTypes, int deviceCount,
    ...
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_GetInputState (
        ovrSession session, ovrControllerType controllerType, ovrInputState *inputState);
    ...
    /// \param[in] frequency Vibration frequency. Supported values are: 0.0 (disabled), 0.5 and 1.0.
    /// Non valid values will be clamped. \param[in] amplitude Vibration amplitude in the [0.0, 1.0]
    /// range. \return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error and
    /// true
    ...
    ovr_SetControllerVibration (
        ovrSession session, ovrControllerType controllerType, float frequency, float amplitude);
    ...
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_SubmitControllerVibration (
        ovrSession session, ovrControllerType controllerType, const ovrHapticsBuffer *buffer);
    ...
    ovr_GetControllerVibrationState (
        ovrSession session, ovrControllerType controllerType, ovrHapticsPlaybackState *outState);
    ...
    ///         upon success. Return values include but aren't limited to:
    ///     - ovrSuccess: The call succeeded and a result was returned.
    ///     - ovrSuccess_BoundaryInvalid: The call succeeded but the result is not a valid boundary
    ///     due to not being set up.
    ///     - ovrSuccess_DeviceUnavailable: The call succeeded but the device referred to by
    ...
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_TestBoundary (ovrSession session, ovrTrackedDeviceType deviceBitmask,
    ...
    /// \param[in] singleBoundaryType Must be either ovrBoundary_Outer or ovrBoundary_PlayArea to
    /// test against \param[out] outTestResult Result of collision/proximity test, contains
    /// information such as distance and closest point. \return Returns an ovrResult for which
    /// OVR_SUCCESS(result) is false upon error and true
    ///         upon success. Return values include but aren't limited to:
    ///     - ovrSuccess: The call succeeded and a result was returned.
    ///     - ovrSuccess_BoundaryInvalid: The call succeeded but the result is not a valid boundary
    ///     due to not being set up.
    ...
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_TestBoundaryPoint (ovrSession session, const ovrVector3f *point,
    ...
    /// Gets the geometry of the Boundary System's "play area" or "outer boundary" as 3D floor
    /// points.
    ///
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    /// \param[in] boundaryType Must be either ovrBoundary_Outer or ovrBoundary_PlayArea.
    ...
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_GetBoundaryGeometry (ovrSession session, ovrBoundaryType boundaryType,
    ...
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    /// \param[in] boundaryType Must be either ovrBoundary_Outer or ovrBoundary_PlayArea.
    /// \param[out] outDimensions Dimensions of the axis aligned bounding box that encloses the area
    /// in meters (width, height and length). \return Returns an ovrResult for which
    /// OVR_SUCCESS(result) is false upon error and true
    ///         upon success. Return values include but aren't limited to:
    ///     - ovrSuccess: The call succeeded and a result was returned.
    ///     - ovrSuccess_BoundaryInvalid: The call succeeded but the result is not a valid boundary
    ...
    ovr_GetBoundaryDimensions (
        ovrSession session, ovrBoundaryType boundaryType, ovrVector3f *outDimensions);
    ...
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_RequestBoundaryVisible (ovrSession session, ovrBool visible);
    ...
    {
        /// ovrLayerFlag_HighQuality enables 4x anisotropic sampling during the composition of the
        /// layer.
        /// The benefits are mostly visible at the periphery for high-frequency & high-contrast
        /// visuals.
        /// For best results consider combining this flag with an ovrTextureSwapChain that has
        /// mipmaps and
    ...
    typedef struct OVR_ALIGNAS (OVR_PTR_SIZE) ovrLayerEyeFov_
    {
        ...
        /// Specifies the position and orientation of each eye view, with position specified in
        /// meters. RenderPose will typically be the value returned from ovr_CalcEyePoses, but can
        /// be different in special cases if a different head pose is used for rendering.
    ...
    ///
    /// \param[in]  session Specifies an ovrSession previously returned by ovr_Create.
    /// \param[in]  chain Specifies the ovrTextureSwapChain for which the length should be
    /// retrieved. \param[out] out_Length Returns the number of buffers in the specified chain.
    ///
    ...
    ovr_GetTextureSwapChainDesc (
        ovrSession session, ovrTextureSwapChain chain, ovrTextureSwapChainDesc *out_Desc);
    ...
    ///
    /// \note When Commit is called, the texture at the current index is considered ready for use by
    /// the runtime, and further writes to it should be avoided. The swap chain's current index is
    /// advanced, providing there's room in the chain. The next time the SDK dereferences this
    /// texture swap chain, it will synchronize with the app's graphics context and pick up the
    /// submitted index, opening up room in the swap chain for further commits.
    ///
    /// \return Returns an ovrResult for which OVR_SUCCESS(result) is false upon error.
    ///         Failures include but aren't limited to:
    ///     - ovrError_TextureSwapChainFull: ovr_CommitTextureSwapChain was called too many times on
    ///     a
    ...
    /// ovr_CreateMirrorTextureWithOptionsDX and ovr_CreateMirrorTextureWithOptionsGL can be found
    /// in rendering API-specific headers, such as OVR_CAPI_D3D.h and OVR_CAPI_GL.h

    /// Destroys a mirror texture previously created by one of the mirror texture creation
    /// functions.
    ///
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    ...
    ovr_GetFovTextureSize (
        ovrSession session, ovrEyeType eye, ovrFovPort fov, float pixelsPerDisplayPixel);
    ...
    ///     - ovrSuccess_NotVisible: rendering of a previous frame completed successfully but was
    ///     not
    ///       displayed on the HMD, usually because another application currently has ownership of
    ///       the HMD. Applications receiving this result should stop rendering new content and call
    ...
    ///       and the shared resources need to be released (ovr_DestroyTextureSwapChain), the
    ///       session needs to destroyed (ovr_Destroy) and recreated (ovr_Create), and new resources
    ///       need to be created (ovr_CreateTextureSwapChainXXX). The application's existing private
    ///       graphics resources do not need to be recreated unless the new ovr_Create call returns
    ///       a different GraphicsLuid.
    ///
    ...
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    ///
    /// \param[in] frameIndex Specifies the targeted application frame index.  It must match what
    /// was
    ...
    ///
    /// \param[in] viewScaleDesc Provides additional information needed only if layerPtrList
    /// contains
    ///        an ovrLayerType_Quad. If NULL, a default version is used based on the current
    ///        configuration and a 1.0 world scale.
    ///
    /// \param[in] layerPtrList Specifies a list of ovrLayer pointers, which can include NULL
    /// entries to
    ...
    ///        version.
    ///
    /// - Layers are drawn in the order they are specified in the array, regardless of the layer
    /// type.
    ///
    ...
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    ///
    /// \param[in] frameIndex Specifies the targeted application frame index, or 0 to refer to one
    /// frame
    ...
    ///         upon success. Return values include but aren't limited to:
    ///     - ovrSuccess: rendering completed successfully.
    ///     - ovrSuccess_NotVisible: rendering completed successfully but was not displayed on the
    ///     HMD,
    ...
    ///
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_SubmitFrame (ovrSession session, long long frameIndex,
    ...
    {
        /// Vsync Frame Index - increments with each HMD vertical synchronization signal (i.e. vsync
        /// or refresh rate) If the compositor drops a frame, expect this value to increment more
        /// than 1 at a time.
        int HmdVsyncIndex;
        ...
        /// If the app fails to call ovr_SubmitFrame on time, then expect this value to increment
        /// with each missed frame
        int AppDroppedFrameCount;

        /// Motion-to-photon latency for the application
        /// This value is calculated by either using the SensorSampleTime provided for the
        /// ovrLayerEyeFov or if that is not available, then the call to ovr_GetTrackingState which
        /// has latencyMarker set to ovrTrue
        float AppMotionToPhotonLatency;

        /// Amount of queue-ahead in seconds provided to the app based on performance and overlap of
        /// CPU and  GPU utilization. A value of 0.0 would mean the CPU & GPU workload is being
        /// completed in 1 frame's worth of time, while 11 ms (on the CV1) of queue ahead would
        /// indicate that the app's CPU workload for the next frame is overlapping the GPU workload
        ...
        /// Index that increments each time the SDK compositor completes a distortion and timewarp
        /// pass Since the compositor operates asynchronously, even if the app calls ovr_SubmitFrame
        /// too late, the compositor will kick off for each vsync.
        int CompositorFrameIndex;

        /// Increments each time the SDK compositor fails to complete in time
        /// This is not tied to the app's performance, but failure to complete can be related to
        /// other factors such as OS capabilities, overall available hardware cycles to execute the
        /// compositor in time and other factors outside of the app's control.
        int CompositorDroppedFrameCount;

        /// Motion-to-photon latency of the SDK compositor in seconds.
        /// This is the latency of timewarp which corrects the higher app latency as well as dropped
        /// app frames.
        float CompositorLatency;

        /// The amount of time in seconds spent on the CPU by the SDK compositor. Unless the
        /// VR app is utilizing all of the CPU cores at their peak performance, there is a good
        /// chance the compositor CPU times will not affect the app's CPU performance in a major
        /// way.
        float CompositorCpuElapsedTime;

        /// The amount of time in seconds spent on the GPU by the SDK compositor. Any time spent on
        /// the compositor will eat away from the available GPU time for the app.
        float CompositorGpuElapsedTime;

        /// The amount of time in seconds spent from the point the CPU kicks off the compositor to
        /// the point in time the compositor completes the distortion & timewarp on the GPU. In the
        /// event the GPU time is not available, expect this value to be -1.0f.
        float CompositorCpuStartToGpuEndElapsedTime;

        /// The amount of time in seconds left after the compositor is done on the GPU to the
        /// associated V-Sync time. In the event the GPU time is not available, expect this value to
        /// be -1.0f.
        ...
        /// Will be true if ASW is active for the given frame such that the application is being
        /// forced into half the frame-rate while the compositor continues to run at full
        /// frame-rate.
        ovrBool AswIsActive;
    ...
    ///
    /// Maximum number of frames of performance stats provided back to the caller of
    /// ovr_GetPerfStats
    ///
    enum
    {
    ...
    typedef struct OVR_ALIGNAS (4) ovrPerfStats_
    {
        ...
        /// ensure that it is calling ovr_SubmitFrame and ovr_GetPerfStats at a rate that is at
        /// least: "HMD_refresh_rate / ovrMaxProvidedFrameStats". On the Oculus Rift CV1 HMD, this
        /// will be equal to 18 times per second.
        ///
    ...
    /// sufficient, will return ovrError_NullArrayPointer. \param[in out] inoutCameraCount Supply
    /// the array capacity, will return the actual # of cameras defined. If *inoutCameraCount is too
    /// small, will return ovrError_InsufficientArraySize. \return Returns the ids of external
    /// cameras the system knows about. Returns
    ...
    OVR_PUBLIC_FUNCTION (ovrResult)
    ovr_SetExternalCameraProperties (ovrSession session, const char *name,
        const ovrCameraIntrinsics *const intrinsics, const ovrCameraExtrinsics *const extrinsics);
    ...
    /// \param[in] valuesCapacity Specifies the maximum number of elements to write to the values
    /// array. \return Returns the number of elements read, or 0 if property doesn't exist or is
    /// empty.
    OVR_PUBLIC_FUNCTION (unsigned int)
    ovr_GetFloatArray (
        ovrSession session, const char *propertyName, float values[], unsigned int valuesCapacity);
    ...
    ///         property name is empty or if the property is read-only.
    OVR_PUBLIC_FUNCTION (ovrBool)
    ovr_SetFloatArray (ovrSession session, const char *propertyName, const float values[],
    ...
    /// \param[in] session Specifies an ovrSession previously returned by ovr_Create.
    /// \param[in] propertyName The name of the property, which needs to be valid only for the call.
    /// \param[in] value The string property, which only needs to be valid for the duration of the
    /// call. \return Returns true if successful, otherwise false. A false result should only occur
    /// if the
    ///         property name is empty or if the property is read-only.
    OVR_PUBLIC_FUNCTION (ovrBool)
